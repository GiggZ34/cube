...
/*   Updated: 2021/01/02 12:40:56 by grivalan         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_modif_map(t_file *file, char **map_checked)
{
	...
	ft_delete_difference(file, map_checked);
	if (!(nb_row = ft_count_array(file->map)))
		ft_error_file(file, 3, "in function ft_modif_map");
	if (file->error_code == 0)
		ft_new_map(file, nb_row);
	return (file->error_code);
}
